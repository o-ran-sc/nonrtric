...
def is_message_new_link_failure(message):
    ...
    if (event_headers["domain"] == "fault"):
        ...
        link_failure = fault_fields["alarmCondition"] == "CUS Link Failure" and fault_fields["eventSeverity"] != "NORMAL"
...
def is_message_clear_link_failure(message):
    ...
    if (event_headers["domain"] == "fault"):
        fault_fields = msg_as_json["event"]["faultFields"]
        link_failure_clear = fault_fields["alarmCondition"] == "CUS Link Failure" and fault_fields["eventSeverity"] == "NORMAL"
...
def handle_link_failure(message, o_ru_to_o_du_map, sdnr_address):
    ...
    response = requests.post(sdnr_address + send_path, json=unlock_msg)
