...
`api_curl.sh` \
A common curl based function for the pms and ics apis. Also partly used for the Callback receiver and RAPP Catalogue apis.
...
Function to load a json config from a file into consul for the PMS
...
# Description of functions in pms_api_functions.sh #
...
## Function: use_pms_rest_http ##

Use http for all API calls to the PMS. This is the default.
| arg list |
|--|
| None |

## Function: use_pms_rest_https ##

Use https for all API calls to the PMS.
| arg list |
|--|
| None |

## Function: use_pms_dmaap_http ##

Send and recieve all API calls to the PMS over Dmaap via the MR over http.
| arg list |
|--|
| None |

## Function: use_pms_dmaap_https ##

Send and recieve all API callss to the PMS over Dmaap via the MR over https.
| arg list |
|--|
| None |
...
Start the PMS container or corresponding kube resources depending on docker/kube mode.
| arg list |
|--|
| `<logfile-prefix>` |
...
## Function: set_pms_debug ##

Configure the PMS log on debug level. The PMS must be running.
| arg list |
|--|
| None |

## Function: set_pms_trace ##

Configure the PMS log on trace level. The PMS must be running.
| arg list |
|--|
| None |

## Function: use_pms_retries ##

Configure the PMS to make upto 5 retries if an API calls return any of the specified http return codes.
| arg list |
|--|
| `[<response-code>]*` |

## Function: check_pms_logs ##

Check the PMS log for any warnings and errors and print the count of each.
| arg list |
|--|
| None |
...
Tests if the array length of a json array in the PMS simulator is equal to a target value.
...
## Function: pms_api_put_policy_batch ##

This tests the same as function 'pms_api_put_policy' except that all put requests are sent to dmaap in one go and then the responses are polled one by one.
If the pms api is not configured to use dmaap (see 'use_pms_dmaap', 'use_pms_rest_http' and 'use_pms_rest_https'), an error message is printed.
For arg list and parameters, see 'pms_api_put_policy'.

## Function: pms_api_put_policy_parallel ##

This tests the same as function 'pms_api_put_policy' except that the policy create is spread out over a number of processes and it only uses the pms rest API. The total number of policies created is determined by the product of the parameters 'number-of-rics' and 'count'. The parameter 'number-of-threads' shall be selected to be not evenly divisible by the product of the parameters 'number-of-rics' and 'count' - this is to ensure that one process does not handle the creation of all the policies in one ric.
...
## Function: pms_api_delete_policy ##
...
## Function: pms_api_delete_policy_batch ##

This tests the same as function 'pms_api_delete_policy' except that all delete requests are sent to dmaap in one go and then the responses are polled one by one.
If the pms api is not configured to used dmaap (see 'use_pms_dmaap', 'use_pms_rest_http' and 'use_pms_rest_https'), an error message is printed.
For arg list and parameters, see 'pms_api_delete_policy'.

## Function: pms_api_delete_policy_parallel ##

This tests the same as function 'pms_api_delete_policy' except that the policy delete is spread out over a number of processes and it only uses the pms rest API. The total number of policies deleted is determined by the product of the parameters 'number-of-rics' and 'count'. The parameter 'number-of-threads' shall be selected to be not evenly divisible by the product of the parameters 'number-of-rics' and 'count' - this is to ensure that one process does not handle the deletion of all the policies in one ric.
...
## Function: pms_api_get_policy_ids ##
...
## Function: pms_api_get_policy_schema ##
...
## Function: pms_api_get_policy_schemas ##
...
## Function: pms_api_get_policy_status ##
...
## Function: pms_api_get_policy_types ##
...
## Function: pms_api_get_service_ids ##
...
## Function: pms_api_delete_services ##
...
## Function: pms_api_put_services_keepalive ##
...
## Function: pms_api_put_configuration ##
...
## Function: pms_api_get_configuration ##
