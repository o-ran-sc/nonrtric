...
This script format API statistics into a test report - data is collected only if the falg `--endpoint-stats` is given to the test script .
...
Depending if the image is a locally build simulator or image or an official image, the following env vara may need to be updated witht the app short name: `PROJECT_IMAGES_APP_NAMES  ORAN_IMAGES_APP_NAMES  ORAN_IMAGES_APP_NAMES`. 
