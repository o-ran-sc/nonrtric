...
#### Function: cr_equal ####
Tests if a variable value in the Callback Receiver (CR) simulator is equal to a target value and an optional timeout.
Without the timeout, the test set pass or fail immediately depending on if the variable is equal to the targer or not.
...
#### Function: mr_equal ####
Tests if a variable value in the Message Router (MR) simulator is equal to a target value and an optional timeout.
Without the timeout, the test set pass or fail immediately depending on if the variable is equal to the targer or not.
With the timeout, the test waits up to the timeout seconds before setting pass or fail depending on if the variable value becomes equal to the target value or not.
...
#### Function: mr_greater ####
Tests if a variable value in the Message Router (MR) simulator is greater than a target value and an optional timeout.
Without the timeout, the test set pass or fail immediately depending on if the variable is greater than the the targer or not.
...
#### Function: mr_read ####
Read the value of a variable in the Message Router (MR) simulator. The value is intended to be passed to a env variable in the test script.
See the 'mrstub' dir for more details.
| arg list |
|--|
| `<variable-name>` |
...
#### Function: mr_print ####
Print the value of a variable in the Message Router (MR) simulator.
See the 'mrstub' dir for more details.
| arg list |
|--|
| `<variable-name>` |
...
Tests if the array length of a json array in the Policy Agent simulator is equal to a target value and an optional timeout.
Without the timeout, the test set pass or fail immediately depending on if the array length is equal to the targer or not.
...
| parameter | description |
| --------- | ----------- |
| `<variable-name>` | Relative url. Example 'json:policy_types' - check the json array length of the url /policy_types  |
...
To test response code only, provide the response code parameter as well as the following three parameter.
To also test the response payload add the 'NOID' for an expected empty array or repeat the last five parameteres for each expected policy.
| arg list |
|--|
...
To the the response code only, provide the expected response code and policy id.
...
| `<ric-id>` | Id the ric  |
| `<policy-type-id>` |  Id of the policy type |
| `<policy-id>` |  Id of the policy. This value shall be a numeric value if more than one policies shall be created |
...
| `<count>` |  An optial count (default is 1). If a value greater than one is given, the policy ids will use the given policy-id as the first id and add 1 to that id for each new policy |

#### Function: api_put_policy_batch() ####
This test the same as function 'api_put_policy' except that all put request as send to dmaap in one go and the the responses are polled one by one.
...
This test the same as function 'api_put_policy' except that the policy create is spread out over a number of processes and it only using the agent rest API.
| arg list |
|--|
...
This test the DELETE /policy. Removes the indicated policy or a 'count' number of policies starting with 'policy-id' as the first id.
| arg list |
|--|
...
This test the same as function 'api_delete_policy' except that all delete request as send to dmaap in one go and the the responses are polled one by one.
If the agent api is not configured to used dmaap (see 'use_agent_dmaap', 'use_agent_rest_http' and 'use_agent_rest_https'), an error message is printed.
...
This test the same as function 'api_delete_policy' except that the policy delete is spread out over a number of processes and it only using the agent rest API.
| arg list |
|--|
| `<response-code> <ric-id-base> <number-of-rics> <policy-start-id> <count-per-ric> <number-of-threads>`
...
Test of GET /ric
To test the response code only, provide the expected response code and management element id.
To test the returned ric id, provide thee expected ric id.
| arg list |
|--|
| `<reponse-code> <management-element-id> [<ric-id>]` |

| parameter | description |
| --------- | ----------- |
| `<response-code>` | Expected http response code |
| `<management-element-id>` |  Id of the management element |
| `<ric-id>` |  Id of the ric |
...
| `<space-separate-string-of-ricinfo>` |  A space separated string of ric info - need to be quoted |
...
The functions below only uses the admin interface of the simulator, no usage of the A1 interface.

#### Function: sim_equal ####
Tests if a variable value in the RIC simulator is equal to a target value and an optional timeout.
Without the timeout, the test set pass or fail immediately depending on if the variable is equal to the targer or not.
With the timeout, the test waits up to the timeout seconds before setting pass or fail depending on if the variable value becomes equal to the target value or not.
...
#### Function: sim_print ####
Print the value of a variable in the RIC simulator.
See the 'a1-interface' repo for more details.
| arg list |
|--|
| `<variable-name>` |
...
Tests if a variable value in the RIC simulator is contains a target string and an optional timeout.
Without the timeout, the test set pass or fail immediately depending on if the variable is contains the targer string or not.
With the timeout, the test waits up to the timeout seconds before setting pass or fail depending on if the variable value contains equal the target sting or not.
See the 'a1-interface' repo for more details.
| arg list |
|--|
| `<variable-name> <target-value> [ <timeout-in-sec> ]` |
...
Load a policy type to the simulator
| arg list |
|--|
...
Delete a policy type from the simulator
| arg list |
|--|
...
Delete instances (and status), for one ric
| arg list |
|--|
...
Delete all type, instances (and status), for one ric
| arg list |
|--|
| `<response-code> <ric-id>` |
...
Set (or reset) response code for next (one) A1 message, for one ric.
...
Set (or reset) A1 response delay, for one ric
The intention is to delayed response on the A1 interface. Setting remains until removed.
| arg list |
|--|
...
The file contains a selected of the possible API tests towards the a1-controller
