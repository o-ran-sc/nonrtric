...
# Generic function to query the PMS/ICS via the REST or DMAAP interface.
# Used by all other PMS/ICS api test functions
...
# arg: (PMS|ICS|CR|RC GET|PUT|POST|DELETE|GET_BATCH|PUT_BATCH|POST_BATCH|DELETE_BATCH <url>|<correlation-id> [<file> [mime-type]]) | (PMS|ICS RESPONSE <correlation-id>)
...
__do_curl_to_api() {
    ...
    if [ $# -gt 0 ]; then
        if [ $1 == "PMS" ]; then
			__ADAPTER=$PMS_ADAPTER
			__ADAPTER_TYPE=$PMS_ADAPTER_TYPE
            __RETRY_CODES=$PMS_RETRY_CODES
    ...
    if [ $paramError -eq 1 ]; then
        ...
        echo "-Expected: (PMS|ICS GET|PUT|POST|DELETE|GET_BATCH|PUT_BATCH|POST_BATCH|DELETE_BATCH <url> [<file> [mime-type]]) | (PMS|ICS RESPONSE <correlation-id>)" >> $HTTPLOG
	...
	if [ $__ADAPTER_TYPE == "REST" ]; then
		...
		if [ $oper == "RESPONSE" ] || [ $batch -eq 0 ]; then
			...
			status=${res:${#res}-3}
			TS=$SECONDS
			# wait of the reply from the PMS/ICS...
