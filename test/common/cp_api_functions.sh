...
__control_panel_export_vars() {
	...
	if [ $RUNMODE == "KUBE" ]; then
		...
		export PMS_DOMAIN_NAME=$PMS_APP_NAME
...
start_control_panel() {
	...
	if [ $RUNMODE == "KUBE" ]; then
		...
		envsubst '${NGW_DOMAIN_NAME},${CP_NGINX_RESOLVER},${NRT_GATEWAY_EXTERNAL_PORT},${PMS_EXTERNAL_SECURE_PORT},${ICS_EXTERNAL_SECURE_PORT},${PMS_DOMAIN_NAME},${ICS_DOMAIN_NAME},${CONTROL_PANEL_PATH_POLICY_PREFIX},${CONTROL_PANEL_PATH_ICS_PREFIX} ,${CONTROL_PANEL_PATH_ICS_PREFIX2}' < $1 > $dest_file
