...
Note that this script will use the staging imaged. Once the release images are available,add the parameter "release" to run with released images.
...
In general, the test scripts support the current ongoing relese as well as two previous releases.
...
KUBE_PRESTARTED_IMAGES=<list of used apps (prestarted - i.e. not started by the script) in this test case - for kube>
