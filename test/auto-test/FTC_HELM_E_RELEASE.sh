...
KUBE_PRESTARTED_IMAGES=" PMS RICSIM CP ICS RC SDNC DMAAPMED DMAAPADP"
...
pms_equal json:rics 6 300

#Check the number of schemas and the individual schemas
pms_equal json:policy-types 5 300

for ((i=0; i<$STD_NUM_RICS; i++))
do
    ricid=$((3+$i))
    pms_equal json:policy-types?ric_id=ric$ricid 1 120
done

for ((i=0; i<$STD_NUM_RICS; i++))
do
   ricid=$((5+$i))
   pms_equal json:policy-types?ric_id=ric$ricid 2 120
done
...
do
   ricid=$((5+$i))
   pms_apiget_policy_type 200 STD_QOS_0_2_0 testdata/STD2/qos-pms-modified.json
   pms_apiget_policy_type 200 STD_QOS2_0.1.0 testdata/STD2/qos2-pms-modified.json
done

# Check the schemas in OSC
for ((i=0; i<$OSC_NUM_RICS; i++))
do
    pms_apiget_policy_type 200 1 testdata/OSC/1-pms-modified.json
    pms_apiget_policy_type 200 2 testdata/OSC/2-pms-modified.json
done
...
if [ "$PMS_VERSION" == "V2" ]; then
    ...
    pms_equal json:policy_schemas 5 120
...
pms_apiput_service 201 "Emergency-response-app" 0 "$CR_SERVICE_APP_PATH_0/ER-app"
...
do
    ...
    pms_apiput_policy 201 "Emergency-response-app" ric$ricid NOTYPE $((1100+$i)) NOTRANSIENT $CR_SERVICE_APP_PATH_0/"std2" testdata/STD/pi1_template.json 1
    generate_policy_uuid
    pms_apiput_policy 201 "Emergency-response-app" ric$ricid NOTYPE $((1200+$i)) NOTRANSIENT $CR_SERVICE_APP_PATH_0/"std2" testdata/STD/pi1_template.json 1
done
...
do
   ricid=$((5+$i))
   generate_policy_uuid
   pms_apiput_policy 201 "Emergency-response-app" ric$ricid STD_QOS_0_2_0 $((2100+$i)) NOTRANSIENT $CR_SERVICE_APP_PATH_0/"std2" testdata/STD2/pi_qos_template.json 1
   generate_policy_uuid
   pms_apiput_policy 201 "Emergency-response-app" ric$ricid STD_QOS2_0.1.0 $((2200+$i)) NOTRANSIENT $CR_SERVICE_APP_PATH_0/"std2" testdata/STD2/pi_qos2_template.json 1
done
...
do
    ricid=$((1+$i))
    generate_policy_uuid
    pms_apiput_policy 201 "Emergency-response-app" ric$ricid 1 $((3100+$i)) NOTRANSIENT $CR_SERVICE_APP_PATH_0/"osc" testdata/OSC/pi1_template.json 1
    generate_policy_uuid
    pms_apiput_policy 201 "Emergency-response-app" ric$ricid 2 $((3200+$i)) NOTRANSIENT $CR_SERVICE_APP_PATH_0/"osc" testdata/OSC/pi2_template.json 1
done
...
if [ "$PMS_VERSION" == "V2" ]; then
    ...
    pms_equal json:policies 12

    pms_equal json:policy-instances 12
else
