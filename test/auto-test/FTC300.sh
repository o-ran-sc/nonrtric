...
for __httpx in $TESTED_PROTOCOLS ; do
    for interface in $TESTED_VARIANTS ; do
        ...
        if [ "$PMS_VERSION" == "V2" ]; then
            pms_equal json:policy-types 2 120  #Wait for the pms to refresh types from the simulator
        else
            pms_equal json:policy_types 2 120  #Wait for the pms to refresh types from the simulator
        fi
        ...
        if [[ $interface == *"BATCH"* ]]; then
            pms_api_put_policy_batch 201 "serv1" ricsim_g2_1 NOTYPE $START_ID2 NOTRANSIENT $notificationurl testdata/STD/pi1_template.json $NUM_POLICIES
        else
            pms_api_put_policy 201 "serv1" ricsim_g2_1 NOTYPE $START_ID2 NOTRANSIENT $notificationurl testdata/STD/pi1_template.json $NUM_POLICIES
        fi
        ...
        pms_api_delete_policy 204 $(($START_ID+47))

        pms_api_delete_policy 204 $(($START_ID+$NUM_POLICIES-39))
        ...
        pms_api_delete_policy 204 $(($START_ID2+37))

        pms_api_delete_policy 204 $(($START_ID2+$NUM_POLICIES-93))

        pms_api_delete_policy 204 $(($START_ID2+$NUM_POLICIES-91))
        ...
        pms_equal json:policies $(($NUM_POLICIES-2+$NUM_POLICIES-3))
