...
public class PolicyControllerMockConfiguration {
	...
	class MockPolicyAgentApi implements PolicyAgentApi {
		...
		public PolicyInstances getPolicyInstancesForType(String type) {
			...
			List<Database.DbPolicyInstance> inst = database.getInstances(Optional.of(type));
			for (Database.DbPolicyInstance i : inst) {
				result.add(new PolicyInstance(i.id, i.json));
			}
			return result;
		}
	...
	class Database {
		...
		private class DbPolicyInstance {
			DbPolicyInstance(PolicyType pt, String id, String json) {
				this.policyType = pt;
				this.json = json;
				this.id = id;
			}
		...
		void putInstance(String typeId, String instanceId, String instanceData) {
			PolicyType t = getType(typeId);
			DbPolicyInstance i = new DbPolicyInstance(t, instanceId, instanceData);
		...
		String getInstance(String id) throws RestClientException {
			DbPolicyInstance i = instances.get(id);
			if (i == null) {
		...
		public List<DbPolicyInstance> getInstances(Optional<String> typeId) {
			ArrayList<DbPolicyInstance> result = new ArrayList<>();
			for (DbPolicyInstance i : instances.values()) {
				if (typeId.isPresent()) {
					if (i.policyType.getName().equals(typeId.get())) {
						result.add(i);
					}
		...
		private PolicyType getType(String typeId) throws RestClientException {
			PolicyType type = types.get(typeId);
			if (type == null) {
				throw new RestClientException("Type not found: " + typeId);
			}
			return type;
		}

		private Map<String, PolicyType> types = new HashMap<>();
		private Map<String, DbPolicyInstance> instances = new HashMap<>();
