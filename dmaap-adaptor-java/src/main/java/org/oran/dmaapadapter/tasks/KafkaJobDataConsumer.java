...
public class KafkaJobDataConsumer {
    ...
    public synchronized void start(Many<String> input) {
        ...
        this.subscription = getMessagesFromKafka(input, job) //
                ...
                .subscribe(this::handleConsumerSentOk, //
                        t -> logger.error("KafkaMessageConsumer jobId: {}, error: {}", job.getId(), t.getMessage()), //
                        () -> this.handleSubscriptionTerminated());
    }
    ...
    private void handleSubscriptionTerminated() {
        logger.error("KafkaMessageConsumer stopped jobId: {}", job.getId());
