...
import org.oran.pmlog.filter.PmReport;
import org.oran.pmlog.tasks.InfluxStore;
import org.oran.pmlog.tasks.KafkaTopicListener.DataFromKafkaTopic;
...
class IntegrationWithKafka {
    ...
    private String generateCounterValue(int sequenceValue, String counterName) {
        long value = (random.nextInt() % 100) * sequenceValue + (counterName.hashCode() % 100);
    ...
    String pmReport(int sequenceValue) {
        try {
            ...
            setEndTime(report, sequenceValue);
            ...
            for (PmReport.MeasInfoList measInfoList : measDataCollection.getMeasInfoList()) {
                for (PmReport.MeasValuesList measValueList : measInfoList.getMeasValuesList()) {
                    for (PmReport.MeasResult measResult : measValueList.getMeasResults()) {
                        String value = this.generateCounterValue(sequenceValue, measType(measResult, measInfoList));
    ...
    @Test
    void storeReportsInflux() throws Exception {
    ...
    void sendPmReportsThroughKafka() throws Exception {
        ...
        var dataToSend = Flux.range(0, NO_OF_OBJECTS).map(i -> kafkaSenderRecord(pmReport(i), "key"));
    ...
    void printConfiguration() {
        ...
        ConsumerJobInfo.KafkaDeliveryInfo deliveryInfo = new ConsumerJobInfo.KafkaDeliveryInfo();
