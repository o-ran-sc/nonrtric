...
class IntegrationWithKafka {
    ...
    String pmReport(int subtractSeconds) {
        try {
            ...
            // Set end time. Now - subtractSeconds
            report.event.getCommonEventHeader()
                    .setLastEpochMicrosec(currentEpochMicroSeconds() - subtractSeconds * 1000 * 1000);
            ...
            for (PmReport.MeasInfoList measInfoList : measDataCollection.getMeasInfoList()) {
                for (PmReport.MeasValuesList measValueList : measInfoList.getMeasValuesList()) {
                    for (PmReport.MeasResult measResult : measValueList.getMeasResults()) {
                        measResult.setSValue(Integer.toString(random.nextInt()));
                    }
                }
            }
    ...
    void kafkaCharacteristics_pmFilter() throws Exception {
        ...
        var dataToSend = Flux.range(1, NO_OF_OBJECTS).map(i -> kafkaSenderRecord(pmReport(i), "key"));
