...
public class ConsumerRegstrationTask {
    ...
    private Mono<ResponseEntity<String>> putInfoJob() {
        try {
            ...
            if (consumerJobInfo.jobId.isBlank() || consumerJobInfo.infoTypeId.isBlank()) {
                logger.info("Skipping creation of infojob. info_type_id or job_id is empty.");
                return Mono.empty();
            }
            String url = applicationConfig.getIcsBaseUrl() + "/data-consumer/v1/info-jobs/" + consumerJobInfo.jobId;
