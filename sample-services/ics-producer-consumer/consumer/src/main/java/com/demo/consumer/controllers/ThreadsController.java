...
public class ThreadsController {
        ...
        try {
            Thread consumerThread = new Thread(() -> {
                try {
                    ...
                    log.error("Error starting consuming on: " + topicName, e);
                }
            });
    ...
    public String stopConsumer() {
        if (consumerThread != null && consumerThread.isAlive()) {
            try {
                ...
                log.error("Error stopping consumer Thread", e);
            }
    ...
    public CompletableFuture<String> listenMessage(@PathVariable int numberOfMessages, @PathVariable String topicName) {
        try {
            Thread consumerThread = new Thread(() -> {
                try {
                    simpleConsumer.run(topicName, new ApplicationMessageHandlerImpl(), numberOfMessages);
                } catch (Exception e) {
                    log.error("Error starting consuming on: " + topicName, e);
                }
            });
