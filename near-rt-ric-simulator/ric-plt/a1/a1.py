...
def put_policy(policyId):
  ...
  if data["policyId"] in list(policy_instances.keys()):
    if data["policyClause"]["scope"] != policy_instances[data["policyId"]]["policyClause"]["scope"]:
      reset_error()
      error["title"] = "The policy already exists with a different scope."
      error["status"] = 404
      error["detail"] = "The policy put involves a modification of the existing scope, which is not allowed."
      params["param"] = "scope"
      error["invalidParams"] = params
      return(error, 404)
  ...
  for i in list(policy_instances.keys()):
    if data["policyClause"] == policy_instances[i]["policyClause"] and \
      ...
      reset_error()
      error["title"] = "The policy already exists with a different id."
      error["status"] = 404
      error["detail"] = "No action has been taken. The id of the existing policy instance is: " + i + "."
      return(error, 404)
  pt = data["policyTypeId"]
  schema = policy_types[pt]
  try:
    print("I promise I'm trying!")
    validate(instance=data, schema=schema)
  except ValueError:
    print("But I failed...")
    reset_error()
    error["title"] = "The json does not validate against the schema."
    error["status"] = 400
    return(error, 400)
