...
def put_policy(policyId):
  ...
  if 'PolicyTypeId' in request.args:
    ...
    schema = policy_types[policyTypeId]
    try:
      validate(instance=data["policySchema"]["policyClause"], schema=schema)
    except:
    ...
    for i in list(policy_instances.keys()):
      if policyId != i and \
         data["policySchema"]["policyClause"] == policy_instances[i]["policySchema"]["policyClause"] and \
  ...
  if policyId in list(policy_instances.keys()):
    if data["policySchema"]["policyClause"]["scope"] != policy_instances[policyId]["policySchema"]["policyClause"]["scope"]:
