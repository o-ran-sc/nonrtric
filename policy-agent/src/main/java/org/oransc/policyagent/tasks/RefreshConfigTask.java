...
import org.oransc.policyagent.configuration.ApplicationConfig.ConfigUpdate;
...
public class RefreshConfigTask {
    ...
    public void start() {
        ...
        refreshTask = createRefreshTask() //
                .subscribe(notUsed -> logger.debug("Refreshed configuration data"),
                        ...
                        () -> logger.error("Configuration refresh completed"));
    }
    ...
    Flux<ApplicationConfig> createRefreshTask() {
        Flux<JsonObject> loadFromFile = Flux.interval(Duration.ZERO, Duration.ofMinutes(1)) //
                .filter(x -> configFileExists()) //
                .filter(x -> this.isConsulUsed == false) //
                .flatMap(x -> loadConfigurationFromFile()) //
        ...
        return Flux.merge(loadFromFile, loadFromConsul) //
                ...
                .doOnNext(this::handleUpdatedConfiguration) //
    ...
    private Flux<ConfigUpdate> updateConfig(ApplicationConfigParser config) {
    ...
    private void handleUpdatedConfiguration(ConfigUpdate updatedInfo) {
        synchronized (this.rics) {
            String ricName = updatedInfo.getRicConfig().name();
            ConfigUpdate.Type event = updatedInfo.getType();
            if (event == ConfigUpdate.Type.ADDED) {
                addRic(updatedInfo.getRicConfig());
            } else if (event == ConfigUpdate.Type.REMOVED) {
                rics.remove(ricName);
                this.policies.removePoliciesForRic(ricName);
            } else if (event == ConfigUpdate.Type.CHANGED) {
    ...
    void runRicSynchronization(Ric ric) {
        RicSynchronizationTask synchronizationTask = new RicSynchronizationTask(a1ClientFactory, policyTypes, policies,
                services);
