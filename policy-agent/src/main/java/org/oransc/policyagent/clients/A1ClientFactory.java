...
public class A1ClientFactory {
    ...
    A1Client createClient(Ric ric, A1ProtocolType version) throws ServiceException {
        if (version == A1ProtocolType.STD_V1_1) {
            ...
            RicConfig ricCfg = ric.getConfig();
            ControllerConfig cntlrlCfg = this.appConfig.getControllerConfig(ricCfg.controllerName());
            return new SdncOscA1Client(version, ricCfg, cntlrlCfg);
        } else if (version == A1ProtocolType.SDNC_ONAP) {
            RicConfig ricCfg = ric.getConfig();
            ControllerConfig cntlrlCfg = this.appConfig.getControllerConfig(ricCfg.controllerName());
            return new SdncOnapA1Client(ric.getConfig(), cntlrlCfg);
        } else {
