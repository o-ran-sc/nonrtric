...
public class ConsumerController {
    ...
    @ApiOperation(value = "Query EI job identifiers", notes = "Returns the identifiers for an EI Type")
    @ApiResponses(
        value = { //
            @ApiResponse(code = 200, message = "EI type", response = String.class, responseContainer = "List"), //
            @ApiResponse(
                code = 404,
                message = "Enrichment Information type is not found",
                response = ErrorResponse.ErrorInfo.class)})
    ...
    @DeleteMapping(
        path = ConsumerConsts.A1E_API_ROOT + "/eitypes/{eiTypeId}/eijobs/{eiJobId}",
        produces = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "Individual EI Job", notes = "Delete an EI job")
    @ApiResponses(
        value = { //
